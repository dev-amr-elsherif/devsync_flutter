...
class _PublicProfileViewState extends State<PublicProfileView> {
  ...
  Widget _buildHeader() {
    ...
    final handle =
        '@${developer.githubUrl?.split('/').last ?? displayName.replaceAll(' ', '')}';
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                    color: AppTheme.secondary.withValues(alpha: 0.8), width: 3),
              ),
              child: Padding(
            ...
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  shape: BoxShape.circle,
                  border: Border.all(
                      color: const Color(0xFF0D121F), width: 4),
                ),
              ),
            ),
          ],
      ...
      children: [
        _buildStatBox('RATING',
            developer.ratingCount > 0
                ? '${developer.avgRating.toStringAsFixed(1)}/5'
                : 'N/A'),
        _buildStatBox('REPOS', developer.publicRepos?.toString() ?? '0'),
        _buildStatBox('PROJECTS',
            developer.topRepositories?.length.toString() ?? '0'),
      ],
    );
  }
  ...
  Widget _buildOverviewTab() {
    final skills =
        (developer.topAiSkills != null && developer.topAiSkills!.isNotEmpty)
      ...
      children: [
        if (projectContext != null)
          DevSyncButton(
            ...
            child: const Text('Send Project Invitation',
                style: TextStyle(
        ...
        OutlinedButton.icon(
          onPressed: () {
            Get.snackbar(
                'Coming Soon', 'Interactive Chat is under development.');
          },
  ...
  Future<void> _handleSendInvitation() async {
    ...
    try {
      final invitation = InvitationModel(
        ...
        status: InvitationStatus.pending, // ✅ FIX
  ...
  Widget _buildReposTab() {
    ...
    if (repos.isEmpty) {
      return const Center(
        child: Text('No GitHub repositories found.',
